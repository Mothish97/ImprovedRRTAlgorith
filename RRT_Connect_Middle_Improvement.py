...
class RRTconnect:
    ...
    def algorithm(self):
        ...
        for i in range(self.iteration):
            ...
            mid_random_node = self.getRandomNode()      
            mid_near_node = self.getNeighborNode(self.mide_node_list,mid_random_node)                  
            is_obstacle_mid,mid_new_node = self.action_set(mid_near_node, mid_random_node)      

           
            if is_obstacle_mid:
                self.mide_node_list.append(mid_new_node)   
                pygame.draw.line(screen, (255,100,0), (screen_scale*mid_near_node.x, screen_scale*100 - screen_scale*mid_near_node.y), (screen_scale*mid_new_node.x, screen_scale*100 - screen_scale*mid_new_node.y),3)

            

           
            distance_S_E, _ = self.getDistanceAngle(end_new_node,start_new_node)
            distance_S_M, _ = self.getDistanceAngle(mid_new_node,start_new_node)
            distance_E_M, _ = self.getDistanceAngle(mid_new_node,end_new_node)

            


            
            if distance_S_M < 1.5 and self.isS_M == False:
                self.isS_M = True
                print("Reached Start to Mid")
                self.path_coord_SM = self.backTrack(mid_new_node,start_new_node)
                for point in self.path_coord_SM:
                    pygame.draw.circle(screen, (255,100,255), (screen_scale*point[0], 100*screen_scale-screen_scale*point[1]), 5)
                    pygame.display.update()

                
              
                
            if distance_E_M<1.5  and self.isE_M == False:
                self.isE_M=True
                print("Reached End to Mid")
                self.path_coord_EM = self.backTrack(end_new_node, mid_new_node)
                for point in self.path_coord_EM:
                    pygame.draw.circle(screen, (255,100,255), (screen_scale*point[0], 100*screen_scale-screen_scale*point[1]), 5)
                    pygame.display.update()
                

            
            if(self.isE_M == True) and (self.isS_M == True):
                print("Goal Reached!!")
                print("Time to Find Path with neighbor: ",time.time() - start_time, "seconds")
                print("Iteration Count : ",iter_count)
                print("Reached using improvements") 
                path_coord_improv = self.path_coord_SM + self.path_coord_EM
